...
<div class="flex flex-col w-full rounded-[30px] border border-[#F1F2F6] p-4 gap-4 bg-white">
    ...
    <div class="flex gap-4">
        ...
        <div class="flex flex-col gap-3 w-full">
            ...
            <hr class="border-[#F1F2F6]">
            <p class="font-semibold text-md text-ngekos-orange">Rp. {{ number_format($room->price_per_month, 0, ',', '.') }}<span
...
<div class="flex flex-col gap-4 pt-[22px]">
    ...
    <div class="flex items-center justify-between">
        ...
        <p class="font-semibold">{{ \Carbon\Carbon::parse($transaction['start_date'])->format('d F Y') }}</p>
    </div>
    <div class="flex items-center justify-between">
        <div class="flex items-center gap-3">
            <img src="/assets/images/icons/calendar.svg" class="w-6 h-6 flex shrink-0" alt="icon">
        ...
        <p class="font-semibold">{{ \Carbon\Carbon::parse($transaction['start_date'])->addMonths(intval($transaction['duration']))->format('d F Y') }}</p>
    </div>
</div>
</div>
<form action="{{ route('payment', $boardingHouse->slug) }}" method="POST" class="relative flex flex-col gap-6 mt-5 pt-5">
    @csrf
<div id="PaymentOptions" class="flex flex-col rounded-[30px] border border-[#F1F2F6] p-5 gap-4 mx-5">
    ...
    @endphp
        <div id="DownPayment-Tab" class="tab-content flex flex-col gap-4">
            ...
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-3">
                    <img src="/assets/images/icons/security-user.svg" class="w-6 h-6 flex shrink-0"
                        alt="icon">
            ...
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-3">
                    <img src="/assets/images/icons/receipt-text.svg" class="w-6 h-6 flex shrink-0"
                        alt="icon">
        ...
        <div id="FullPayment-Tab" class="tab-content flex flex-col gap-4 hidden">
            ...
            <div class="flex items-center justify-between">
                ...
                <p id="fullPaymentPrice" class="font-semibold">Rp. {{ number_format($grandTotal, 0, ',', '.') }}</p>
            </div>
        </div>
    </div>
</div>
