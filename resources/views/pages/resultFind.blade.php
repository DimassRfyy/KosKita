...
<div id="TopNav" class="relative flex items-center justify-between px-5 mt-[60px]">
    <a href="{{ route('find') }}" class="w-12 h-12 flex items-center justify-center shrink-0 rounded-full overflow-hidden bg-white">
        ...
        <a href="{{ route('details', $boardingHouse->slug) }}" class="card">
            <div class="flex rounded-[30px] border border-[#F1F2F6] p-4 gap-4 bg-white hover:border-[#91BF77] transition-all duration-300">
                ...
                <div class="flex flex-col gap-3 w-full">
                    ...
                    <hr class="border-[#F1F2F6]">
                    <p class="font-semibold text-md text-ngekos-orange">Rp. {{ number_format($boardingHouse->price, 0, ',', '.') }}<span class="text-xs text-ngekos-grey font-normal">/bulan</span></p>
                </div>
            </div>
        </a>
    @endforeach
