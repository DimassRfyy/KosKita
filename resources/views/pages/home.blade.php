...
<div class="flex flex-col gap-1">
    <img src="assets/images/photos/logo.png" height="150" width="150" alt="">
</div>
<a href="#"
...
<div class="swiper-wrapper">
    ...
    <div class="swiper-slide !w-fit pb-[30px]">
        <a href="{{ route('kosByCategory', $category->slug) }}" class="card">
            <div
...
<div class="swiper w-full overflow-x-hidden">
    <div class="swiper-wrapper">
       ...
       @foreach ($popularBoardingHouses as $popularKos)
       <div class="swiper-slide !w-fit">
        <a href="{{ route('details', $popularKos->slug) }}" class="card">
            <div
                ...
                <div class="flex w-full h-[150px] shrink-0 rounded-[30px] bg-[#D9D9D9] overflow-hidden">
                    <img src="{{ Storage::url($popularKos->thumbnail) }}" class="w-full h-full object-cover"
                        alt="thumbnail">
                </div>
                <div class="flex flex-col gap-3">
                    <h3 class="font-semibold text-lg leading-[27px] line-clamp-2 min-h-[54px]">{{ $popularKos->name }}</h3>
                    <hr class="border-[#F1F2F6]">
                    <div class="flex items-center gap-[6px]">
                        <img src="assets/images/icons/location.svg" class="w-5 h-5 flex shrink-0"
                            alt="icon">
                        <p class="text-sm text-ngekos-grey">{{ $popularKos->city->name }}</p>
                    </div>
                    <div class="flex items-center gap-[6px]">
                        <img src="assets/images/icons/3dcube.svg" class="w-5 h-5 flex shrink-0"
                            alt="icon">
                        <p class="text-sm text-ngekos-grey">{{ $popularKos->category->name }}</p>
                    </div>
                    <div class="flex items-center gap-[6px]">
                    ...
                    <hr class="border-[#F1F2F6]">
                    <p class="font-semibold text-lg text-ngekos-orange">Rp. {{ number_format($popularKos->price, 0, ',', '.') }}<span
...
<div class="grid grid-cols-2 gap-4">
   ...
   <a href="{{ route('kosByCity', $city->slug) }}" class="card">
    <div
...
<div class="flex items-center justify-between">
    <h2 class="font-bold">All Great KosKita</h2>
    <a href="#">
        <div class="flex items-center gap-2">
            <span>See all</span>
            <img src="assets/images/icons/arrow-right.svg" class="w-6 h-6 flex shrink-0" alt="icon">
...
<div class="flex flex-col gap-4">
   ...
   <a href="{{ route('details', $boardingHouse->slug) }}" class="card">
    <div
