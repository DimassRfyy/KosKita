...
<div class="flex flex-col w-full rounded-[30px] border border-[#F1F2F6] p-4 gap-4 bg-white">
    ...
    <div class="flex gap-4">
        <div class="flex w-[120px] h-[156px] shrink-0 rounded-[30px] bg-[#D9D9D9] overflow-hidden">
            <img src="{{ Storage::url($room->images->first()->image) }}" class="w-full h-full object-cover" alt="icon">
        </div>
        <div class="flex flex-col gap-3 w-full">
...
<form action="{{ route('information_save', $boardingHouse->slug) }}" method="POST" class="relative flex flex-col gap-6 mt-5 pt-5 bg-[#F5F6F8]">
    @csrf
...
<div id="InputContainer" class="flex flex-col gap-[18px]">
    <div class="flex flex-col w-full gap-2 px-5">
       ...
       placeholder="Write your name" value="{{ old('name') }}" required>
        </label>
    </div>
    <div class="flex flex-col w-full gap-2 px-5">
        ...
        <label
            ...
            <input type="email" name="email" id=""
                class="appearance-none outline-none w-full font-semibold placeholder:text-ngekos-grey placeholder:font-normal"
                placeholder="Write your email" value="{{ old('email') }}" required>
        </label>
    </div>
    <div class="flex flex-col w-full gap-2 px-5">
        ...
        <label
            ...
            <input type="tel" name="phone_number" id=""
                class="appearance-none outline-none w-full font-semibold placeholder:text-ngekos-grey placeholder:font-normal"
                placeholder="Write your phone" value="{{ old('phone_number') }}" required>
        </label>
    </div>
